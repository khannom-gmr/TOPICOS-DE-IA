...
// Input 1x28x28
//  -> Conv1 (16 filtros 5x5)        -> 16x24x24
//  -> ReLU -> MaxPool 2x2           -> 16x12x12
//  -> Conv2 (32 filtros 5x5)        -> 32x8x8
//  -> ReLU -> MaxPool 2x2           -> 32x4x4
//  -> Flatten                       -> 512
//  -> FC1 512->128 -> ReLU
//  -> FC2 128->10  -> Softmax
class CNN_CUDA {
public:
    ...
    // csvPath: si no es vacio, guarda epoch,loss,accuracy en ese archivo al terminar
    void train(const std::vector<float>& X, const std::vector<int>& labels,
               int numSamples, int epochs, float lr, int batchSize,
               const std::string& csvPath = "training_log.csv");
    float computeAccuracy(const std::vector<float>& X,
                          const std::vector<int>& labels, int numSamples);

    // Imprime accuracy, precision/recall/F1 por clase, macro/ponderado,
    // matriz de confusion y la perdida promedio. Devuelve la perdida promedio.
...
private:
    ...
    static const int C1_OUT = 16, C1_K = 5, C1_H = 24, C1_W = 24; // tras conv1
    static const int P1_H = 12, P1_W = 12;                        // tras pool1
    static const int C2_OUT = 32, C2_K = 5, C2_H = 8,  C2_W = 8;  // tras conv2
