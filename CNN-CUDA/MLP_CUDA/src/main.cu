...
int main() {
    ...
    // El CNN tiene mas capacidad y converge mas lento por epoca (procesa imagen
    // por imagen en las capas conv) -> le damos mas epocas para llegar a ~98%.
    ...
    cnn.evaluate(testX, testY, nTest); // reporte completo: precision/recall/F1 + matriz de confusion
