...
const int ESPACIADO_CUADRICULA = 30;
...
bool usar_bfs = false;
ModoInteraccion modo_interaccion = ModoInteraccion::Ruta;
...
const char* nombreModoInteraccion() {
    return modo_interaccion == ModoInteraccion::Obstaculos ? "Obstaculos" : "Ruta";
}
...
EjecucionDijkstra ejecutarBusquedaDijkstra(int origen, int destino) {
    vector<NodoCuadricula> cuadricula;
    cuadricula.reserve(nodos.size());
...
EjecucionBFS ejecutarBusquedaBFS(int origen, int destino) {
    vector<NodoCuadricula> cuadricula;
    cuadricula.reserve(nodos.size());
    ...
    return ejecutarBFS(cuadricula, total_columnas, total_filas, origen, destino, true);
}
...
void limpiarObstaculosMarcados() {
    for (auto& nodo : nodos) {
        nodo.eliminado = false;
    }
}
...
void refrescarRuta() {
    ...
    if (usar_dijkstra) {
        EjecucionDijkstra ejecucion = ejecutarBusquedaDijkstra(id_inicio, id_fin);
        orden_generados = ejecucion.orden_generados;
        orden_expandidos = ejecucion.orden_expandidos;
        ruta_pendiente = ejecucion.ruta;
    } else if (usar_bfs) {
        EjecucionBFS ejecucion = ejecutarBusquedaBFS(id_inicio, id_fin);
        orden_generados = ejecucion.orden_generados;
        orden_expandidos = ejecucion.orden_expandidos;
        ruta_pendiente = ejecucion.ruta;
    } else {
        EjecucionAEstrella ejecucion = ejecutarBusquedaAEstrella(id_inicio, id_fin);
        orden_generados = ejecucion.orden_generados;
        orden_expandidos = ejecucion.orden_expandidos;
        ruta_pendiente = ejecucion.ruta;
    }
...
void alternarObstaculoEn(int id_nodo) {
    if (id_nodo < 0 || id_nodo >= static_cast<int>(nodos.size())) {
        return;
    }

    if (id_nodo == id_inicio || id_nodo == id_fin) {
        return;
    }

    nodos[id_nodo].eliminado = !nodos[id_nodo].eliminado;
    refrescarRuta();
}
...
void callbackBotonMouse(GLFWwindow* ventana, int boton, int accion, int) {
    ...
    if (modo_interaccion == ModoInteraccion::Obstaculos) {
        if (boton == GLFW_MOUSE_BUTTON_LEFT) {
            alternarObstaculoEn(id_seleccionado);
        }
        return;
    }
    ...
    if (boton == GLFW_MOUSE_BUTTON_LEFT) {
        id_inicio = id_seleccionado;
    } else {
        id_fin = id_seleccionado;
    }
...
void reiniciarCuadriculaYExtremos() {
    construirCuadricula();
    id_inicio = 0;
    id_fin = static_cast<int>(nodos.size()) - 1;
    refrescarRuta();
}
...
void dibujarEscena() {
    ...
    const float medio = ESPACIADO_CUADRICULA * 0.5f;

    for (const auto& nodo : nodos) {
        const float izquierda = nodo.x - medio;
        const float derecha = nodo.x + medio;
        const float arriba = nodo.y - medio;
        const float abajo = nodo.y + medio;
        ...
        glBegin(GL_QUADS);
        glVertex2f(izquierda, arriba);
        glVertex2f(derecha, arriba);
        glVertex2f(derecha, abajo);
        glVertex2f(izquierda, abajo);
        glEnd();
    ...
    for (const auto& nodo : nodos) {
        const float izquierda = nodo.x - medio * 0.45f;
        const float derecha = nodo.x + medio * 0.45f;
        const float arriba = nodo.y - medio * 0.45f;
        const float abajo = nodo.y + medio * 0.45f;
...
void callbackTeclado(GLFWwindow* ventana, int tecla, int, int accion, int) {
    ...
    if (tecla == GLFW_KEY_E) {
        modo_interaccion = ModoInteraccion::Obstaculos;
        std::cout << "Modo: Obstaculos (click izquierdo para alternar celdas negras)" << std::endl;
    }
    if (tecla == GLFW_KEY_S) {
        modo_interaccion = ModoInteraccion::Ruta;
        std::cout << "Modo: Ruta (click izquierdo inicio, click derecho destino)" << std::endl;
    }
    if (tecla == GLFW_KEY_D) {
        usar_dijkstra = !usar_dijkstra;
        std::cout << "Usando algoritmo: " << (usar_dijkstra ? "Dijkstra" : "A*") << std::endl;
        refrescarRuta();
    }
    if (tecla == GLFW_KEY_B) {
        usar_bfs = !usar_bfs;
        if (usar_bfs) usar_dijkstra = false;
        std::cout << "Usando algoritmo: " << (usar_bfs ? "BFS" : (usar_dijkstra ? "Dijkstra" : "A*")) << std::endl;
        refrescarRuta();
    }
    if (tecla == GLFW_KEY_C) {
        limpiarObstaculosMarcados();
        refrescarRuta();
    }
...
int runAsteriscoApp() {
    ...
    cout << "  E: Modo editar obstaculos" << endl;
    cout << "  S: Modo escoger ruta" << endl;
    cout << "  Espacio: Recalcular ruta" << endl;
    cout << "  D: Alternar Dijkstra" << endl;
    cout << "  B: Alternar BFS" << endl;
